...
class MNIST_MLP(object):
    ...
    def build_model(self, batch_size=100, input_size=784, 
                    hidden1=400, hidden2=100, hidden3=32, out_classes=10,
                    quant_param_path='../../mnist_mlp_quant_param.npz'):    # 建立网络结构  ##未修改
        ...
        # fc3
        self.net.createMlpLayer('fc3', hidden3, self.input_quant_params[2])
        # relu3
        self.net.createReLuLayer('relu3')
        # fc4
        self.net.createMlpLayer('fc4', out_classes, self.input_quant_params[2])
        # softmax
    ...
    def load_model(self, param_dir):  # 加载参数
        ...
        weigh4 = np.transpose(params['w4'], [1, 0]).flatten().astype(np.float64)
        bias4 = params['b4'].flatten().astype(np.float64)
        self.net.loadParams(6, weigh4, bias4, self.filter_quant_params[2])
...
def run_mnist():
    batch_size = 10000
    h1, h2, h3,c = HIDDEN1, HIDDEN2, HIDDEN3,OUT
    mlp = MNIST_MLP()
    mlp.build_model(batch_size=batch_size, hidden1=h1, hidden2=h2, hidden3=h3,out_classes=c)
