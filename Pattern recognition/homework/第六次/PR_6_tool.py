...
class Kmeans():
    ...
    def generate_center(self, x):
        ...
        :process
            ①  先将每个样本进行模长归一化，然后计算样本点对的余弦值(直接点积),计算每个样本点的密度(通过样本点cos值top10 max累加得到--无自己与自己点积)
                选择密度最大的那个样本作为第一个初始聚类中心，添加到u_list  --- new version
               随机挑选一个样本作为一个聚类中心，并添加到u_list  --- old version
        ...
        N = x.shape[0]  # 样本数
        u_list = []
        # u1 = x[int(N * np.random.random_sample()), :].reshape((1, x.shape[1]))  # ① 随机获得一个聚类中心  # old version
        norm = np.linalg.norm(x , axis=1, keepdims=True)
        index_zero = np.argwhere(norm == 0) # 防止零向量的影响
        x_norm = x / norm  # 归一化过程
        x_norm[index_zero, :] = 0   # 防止零向量的影响
        x_cos = np.dot(x_norm,x_norm.T)
        p = np.zeros(x_cos.shape[0])
        for i in range(x_cos.shape[0]):
            p[i]=x_cos[i,(-x_cos[i,:]).argsort()[1:10+1]].sum()
        u1=x[p.argmax(),:]  # new version
        u_list.append(u1)
